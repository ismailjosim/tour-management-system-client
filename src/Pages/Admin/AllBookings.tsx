  ...
  const columns = [
    {
      ...
      render: (tour: Booking['tour']) => (
        <div className="flex items-center gap-2">
          ...
          <div>
            <div className="max-w-[200px] truncate font-medium">{tour.title}</div>
            <div className="text-muted-foreground text-xs">{tour.location}</div>
          </div>
        </div>
      ),
    },
    ...
    },
    {
      ...
      render: (payment: Booking['payment']) => (
        <div className="hidden md:block">
          ...
          <div
            className={`text-xs ${payment.status === 'PAID' ? 'text-green-600' : 'text-red-600'}`}
          >
            {payment.status}
          </div>
        </div>
    ...
    },
    {
      ...
      render: (createdAt: string) => (
        <div className="hidden lg:table-cell">{format(new Date(createdAt), 'MMM dd, yyyy')}</div>
      ),
    },
    {
      key: 'actions',
