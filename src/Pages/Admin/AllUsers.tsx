...
const AllUsers = () => {
  ...
  const [updateUserInfoViaAdmin, { isLoading: isUpdating }] = useUpdateUserInfoViaAdminMutation();
  ...
  const handleActiveStatusUpdate = async (userId: string, isActive: IsActive) => {
    try {
      const result = await updateUserInfoViaAdmin({
        userId,
        isActive,
      ...
      if (result.success && result.statusCode === 201) {
        const statusText =
          isActive === 'ACTIVE' ? 'activated' : isActive === 'BLOCKED' ? 'blocked' : 'deactivated';
  ...
  const handleDeleteStatusUpdate = async (userId: string, isDeleted: boolean) => {
    try {
      const result = await updateUserInfoViaAdmin({
        userId,
        isDeleted,
    ...
    },
    {
      ...
      render: (name: string, item: IUser) => (
        <div className="font-medium">
          ...
          <div
            className="text-muted-foreground hidden max-w-[150px] truncate text-sm sm:table-cell"
            title={item.email}
          >
            {item.email}
          </div>
        ...
        <div className="hidden sm:table-cell">
          <div className="flex flex-col justify-center gap-1">
            ...
            {item.isVerified ? (
              ...
              <Badge
                variant="outline"
                className="text-destructive border-destructive w-fit text-xs"
              >
                Not Verified
              </Badge>
            )}
        ...
        <div className="hidden md:table-cell">
          {item.address && item.phone ? (
            <div className="text-sm">
              ...
              {item.address && (
                <div className="text-muted-foreground truncate" title={item.address}>
                  {item.address}
                </div>
              )}
            </div>
          ) : (
            <div className="text-muted-foreground max-w-[120px] truncate" title="Info Not Found">
              Info Not Found
            </div>
          )}
        </div>
            ...
            <div className="flex items-center justify-end gap-2">
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button size="sm" variant="outline" disabled={isUpdating || isSuperAdmin}>
                ...
                <DropdownMenuContent align="end" className="w-56">
                  ...
                  <DropdownMenuItem
                    onClick={() => handleActiveStatusUpdate(item._id, 'INACTIVE')}
                  ...
                  <DropdownMenuItem
                    onClick={() => handleActiveStatusUpdate(item._id, 'BLOCKED')}
                  ...
                  <DropdownMenuItem
                    onClick={() => handleVerificationUpdate(item._id, !item.isVerified)}
                  ...
                  <DropdownMenuItem
                    onClick={() => handleDeleteStatusUpdate(item._id, !item.isDeleted)}
                    className={item.isDeleted ? 'text-green-600' : 'text-red-600'}
                    disabled={isSuperAdmin}
                  >
                    {item.isDeleted ? (
                      <>
