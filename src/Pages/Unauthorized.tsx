      ...
      <Card className="w-[90%] max-w-lg bg-zinc-800 text-center shadow-2xl transition-all duration-300 hover:scale-105 sm:p-4">
        <CardHeader className="space-y-4">
          <div className="flex justify-center">
            <ShieldAlert className="h-16 w-16 animate-pulse text-red-500" />
          </div>
        ...
        <CardContent className="flex flex-col items-center justify-center space-y-4">
          ...
          <Button className="mt-4 w-full rounded-full bg-blue-600 px-8 py-3 font-semibold text-white shadow-lg transition-transform duration-300 hover:scale-105 hover:bg-blue-700 focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:outline-none">
            Back To Home
          </Button>
        </CardContent>
      </Card>
...
const Card = ({ className, ...props }: ComponentProps) => (
  <div className={`rounded-xl border border-zinc-700 bg-zinc-800 p-6 ${className}`} {...props} />
);
const CardHeader = ({ className, ...props }: ComponentProps) => (
  <div className={`flex flex-col space-y-1.5 p-6 ${className}`} {...props} />
);
const CardTitle = ({ className, ...props }: ComponentProps) => (
  <h3 className={`text-2xl leading-none font-semibold tracking-tight ${className}`} {...props} />
);
...
const Button = ({ className, ...props }: ComponentProps) => (
  <Link
    to={'/'}
    className={`focus-visible:ring-ring ring-offset-background inline-flex h-10 items-center justify-center rounded-md px-4 py-2 text-sm font-medium transition-colors focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:outline-none disabled:pointer-events-none disabled:opacity-50 ${className}`}
    {...props}
  />
);
