...
const UserActions = ({ user }: UserActionsProps) => {
  ...
  const handleDeleteStatusUpdate = async (isDeleted: boolean) => {
    try {
      ...
      if (result.success && result.statusCode === 201) {
        toast.success(`User ${isDeleted ? 'marked as deleted' : 'restored'} successfully`);
      }
    } catch (error: any) {
    ...
    <div className="flex items-center justify-end gap-2">
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="w-56">
          {/* Role Management */}
          <DropdownMenuItem className="text-muted-foreground px-2 py-1 text-xs font-medium">
            ROLE MANAGEMENT
          </DropdownMenuItem>
          <DropdownMenuItem
          ...
          <DropdownMenuItem onClick={() => handleVerificationUpdate(!user.isVerified)}>
            {user.isVerified ? (
              <>
