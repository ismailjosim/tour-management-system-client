...
const BookingDetailsModal = ({ booking, trigger }: BookingDetailsModalProps) => {
  return (
    <Dialog>
      ...
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>Full details of booking ID: {booking._id}</DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="text-muted-foreground text-sm">{booking.tour.location}</p>
              <p className="text-sm">
                {format(new Date(booking.tour.startDate), 'MMM dd, yyyy')} -{' '}
                {format(new Date(booking.tour.endDate), 'MMM dd, yyyy')}
              </p>
              <p className="text-sm font-medium">Price: ${booking.tour.costFrom}</p>
            </div>
          </div>

          {/* User Info */}
          <div>
            <h5 className="mb-1 font-semibold">Booked By</h5>
            <p>{booking.user.name}</p>
            <p className="text-muted-foreground text-sm">{booking.user.email}</p>
          </div>
          ...
          <div>
            ...
            <p>
              Transaction ID: <span className="font-mono">{booking.payment.transactionId}</span>
            </p>
            <p>
