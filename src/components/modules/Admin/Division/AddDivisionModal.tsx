      ...
      <Dialog open={open} onOpenChange={setOpen}>
        {triggerButton ? (
          ...
          <DialogTrigger asChild>
            <Button variant="default">{isEditMode ? 'Edit Division' : 'Add Division'}</Button>
          </DialogTrigger>
        )}

        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle>{isEditMode ? 'Edit Division' : 'Add New Division'}</DialogTitle>
            <DialogDescription>
          ...
          <Form {...form}>
            <form className="space-y-5" onSubmit={handleSubmit(onSubmit)}>
              ...
              >
                {(adding || updating) && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
                {adding || updating
                  ? isEditMode
                    ? 'Updating...'
                    : 'Submitting...'
