...
export default function TourForm({ mode, initialData, tourId }: TourFormProps) {
  ...
  useEffect(() => {
    if (mode === 'edit' && initialData) {
      form.reset({
        ...
        endDate: initialData.endDate ? parseISO(initialData.endDate) : new Date(),
  ...
  const handleSubmit = async (data: z.infer<typeof addTourSchema>) => {
    const toastId = toast.loading(mode === 'create' ? 'Creating tour...' : 'Updating tour...');
    ...
    const existingImages = images.filter((img): img is string => typeof img === 'string');
    ...
    try {
      ...
      if (res.success) {
        toast.success(
          mode === 'create' ? 'Tour created successfully!' : 'Tour updated successfully!',
          { id: toastId }
        );
        if (mode === 'create') {
          form.reset();
      ...
      toast.error(
        apiError.data?.message || `Error ${mode === 'create' ? 'creating' : 'updating'} tour`,
        { id: toastId }
      );
    }
  };
  ...
  return (
    <div className="mx-auto w-full max-w-4xl sm:mt-16">
      <Card>
        <CardHeader>
          <CardTitle>{mode === 'create' ? 'Add New Tour' : 'Edit Tour'}</CardTitle>
          <CardDescription>
            {mode === 'create' ? 'Add a new tour to the system' : 'Update tour information'}
          </CardDescription>
        </CardHeader>

        <CardContent>
          <Form {...form}>
            <form id="tour-form" className="space-y-5" onSubmit={form.handleSubmit(handleSubmit)}>
              <TourBasicInfo form={form} />

              {/* Departure Location Map */}
              <div className="space-y-2">
                <label className="block text-sm font-medium">Departure Location (Map)</label>
                <LocationInput
                  onSelect={(location) => {
              ...
              <div className="space-y-2">
                <label className="block text-sm font-medium">Arrival Location (Map)</label>
                <LocationInput
                  onSelect={(location) => {
              ...
              <div className="flex flex-col items-stretch gap-5 sm:flex-row">
                ...
                <div className="mt-5 flex-1">
                  <MultipleImageUploader
                    onChange={setImages}
                    initialImages={mode === 'edit' && initialData?.images ? initialData.images : []}
                  />
                </div>
              </div>
            </form>
